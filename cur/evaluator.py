...
class Evaluator:
    def __init__(self, ground_truth_fname):
        self.origin_ranking = np.matrix(np.genfromtxt(ground_truth_fname,
                                                      delimiter=',',
                                                      dtype=float)).T

    def evaluate(self, generated_ranking):
        tau, tau_pvalue = sps.kendalltau(generated_ranking,
                                         self.origin_ranking)

        rho, rho_pvalue = sps.spearmanr(generated_ranking,
                                        self.origin_ranking, axis=None)

        mae = mean_absolute_error(generated_ranking, self.origin_ranking)
        return {'mae': mae, 'tau': [tau, tau_pvalue], 'rho':
            [rho, rho_pvalue]}
    ...
    def compare(self, current_performance, origin_performance):
        ...
        for metric_name in origin_performance.iterkeys():
            if metric_name == 'tau' or metric_name == 'rho':
                if current_performance[metric_name] > origin_performance[
                    metric_name]:
...
class FoldEvaluator:
    ...
    def evaluate(self, generate_ranking_fold):
        ...
        return {'mae': average_performance[0],
    ...
    def _evaluate(self, generated_ranking, fold_index):
        ...
        mae = mean_absolute_error(test_generated, test_ground_truth)
        return np.asarray([mae, tau, tau_pvalue, rho, rho_pvalue],
                          dtype=float)
    ...
    def single_fold_evaluate(self, generated_ranking, fold_index):
        ...
        return {'mae': mae, 'tau': [tau, tau_pvalue], 'rho': [rho, rho_pvalue]}
